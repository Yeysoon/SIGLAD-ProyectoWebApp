...
export async function login(req, res, next) {
  try {
    ...
    if (!email || !password) {
      return res
        .status(400)
        .json({ ok: false, error: "Email y password requeridos" });
    }
    ...
    const sqlQuery = `
      ...
      LIMIT 1
    `;
    const {  rows: rowsInactivo } = await pool.query(sqlQuery, [email, password]);

    if (rowsInactivo.length > 0) {
      // Bitácora fallo
      await pool.query(
    ...
    /* Se comenta esta seccion que es necesaria modificarla para poder implementar  la informacion y 
    agregar el gmail para identificar el nombre del usuarios.
    // Token (2h)
    const token = jwt.sign(
